SELECT * FROM `sage-archway-451722-t5.sql_practice.user_events` LIMIT 1000;
...
WITH funnel_stages AS (
  ...
  FROM `sage-archway-451722-t5.sql_practice.user_events`
...
SELECT
  ...
  payments,
  purchases,
  ROUND(carts * 100 / views) AS cart_convertion_rate,
  ROUND(checkouts * 100 / carts) AS checkout_convertion_rate,
  ROUND(payments * 100 / checkouts) AS payment_convertion_rate,
  ROUND(purchases * 100 / payments) AS purchase_convertion_rate,
  ROUND(purchases * 100 / views) AS total_convertion_rate,
FROM source_funnel
